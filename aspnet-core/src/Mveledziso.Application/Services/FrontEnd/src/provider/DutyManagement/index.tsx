...
export const UserDutyProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const createUserDuty = async (duty: ICreateUserDutyDto): Promise<IUserDuty> => {
    console.log('createUserDuty called with:', duty);
    dispatch(setPending());
    try {
      ...
      console.log('Formatted duty:', formattedDuty);

      // Validate GUIDs
      if (!isValidGuid(formattedDuty.teamMemberId)) {
        console.error('Invalid teamMemberId GUID:', formattedDuty.teamMemberId);
        throw new Error('Invalid GUID format for teamMemberId');
      }
      if (!isValidGuid(formattedDuty.projectDutyId)) {
        console.error('Invalid projectDutyId GUID:', formattedDuty.projectDutyId);
        throw new Error('Invalid GUID format for projectDutyId');
      }

      console.log('Sending API request to:', `${API_ENDPOINTS.userDuties}/Create`);
      console.log('Request payload:', formattedDuty);
      ...
      console.error('Error in createUserDuty:', error);
