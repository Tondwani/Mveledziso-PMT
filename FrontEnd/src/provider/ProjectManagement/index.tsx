...
export const ProjectProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const createProject = async (project: ICreateProjectDto) => {
    ...
    try {
      ...
      dispatch(projectError(apiError.response?.data?.error?.message || "Project creation failed"));
      throw error;
    }
  };
  ...
  const updateProject = async (project: IUpdateProjectDto): Promise<IProject> => {
    ...
    try {
      ...
      dispatch(createProjectSuccess(updatedProject));
      return updatedProject;
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Project update failed"));
      throw error;
    }
  };
  ...
  const getProject = async (id: string): Promise<IProject> => {
    ...
    try {
      ...
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Failed to load project"));
      throw error;
    }
  };
  ...
  const getProjects = async (input: IGetProjectsInput): Promise<IProject[]> => {
    ...
    try {
      ...
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Failed to load projects"));
      throw error;
    }
  };
  ...
  const getProjectWithDetails = async (id: string): Promise<IProject> => {
    ...
    try {
      ...
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Failed to load project details"));
      throw error;
    }
  };
  ...
  const createProjectDuty = async (duty: ICreateProjectDutyDto) => {
    dispatch(dutyPending());
    try {
      const response = await instance.post(`${API_ENDPOINTS.duties}/Create`, duty);
      const createdDuty: IProjectDuty = response.data.result;
      dispatch(createDutySuccess(createdDuty));
      return createdDuty;
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Duty creation failed"));
      throw error;
    }
  };
  ...
  const updateProjectDuty = async (duty: IUpdateProjectDutyDto): Promise<IProjectDuty> => {
    ...
    try {
      ...
      dispatch(createDutySuccess(updatedDuty));
      return updatedDuty;
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Duty update failed"));
      throw error;
    }
  };
  ...
  const updateDutyStatus = async (id: string, status: DutyStatus) => {
    ...
    try {
      ...
      dispatch(createDutySuccess(updatedDuty.data.result));
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Status update failed"));
      throw error;
    }
  };
  ...
  const getProjectDuty = async (id: string): Promise<IProjectDuty> => {
    ...
    try {
      ...
      dispatch(createDutySuccess(duty));
      return duty;
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Failed to load duty"));
      throw error;
    }
  };
  ...
  const getProjectDuties = async (input: IGetProjectDutiesInput): Promise<IProjectDuty[]> => {
    ...
    try {
      ...
      dispatch(loadDutiesSuccess(duties));
      return duties;
    } catch (error: unknown) {
      const apiError = error as ApiError;
      dispatch(projectError(apiError.response?.data?.error?.message || "Failed to load duties"));
      throw error;
    }
  };
